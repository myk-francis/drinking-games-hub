...
test("flip 7 is listed with a two-player minimum for self-service", () => {
  assert.equal(MIN_PLAYERS_BY_GAME["flip-7"], 2);
});
...
test("self-service validation blocks flip 7 rooms with fewer than two players", () => {
  const error = getSelfServiceValidationError({
    version: 1,
    selectedGame: "flip-7",
    players: ["A"],
